...
using UnityEngine;
using StatsSystem = Hiro.StatsSystem;

namespace UnityGacha.Tests.Editor
{
    /// <summary>
    ...
    /// </summary>
    [TestFixture]
    public class GachaControllerTests
    {
        ...
        public async Task SetUp()
        {
            ...
            _statsSystem = new StatsSystem(logger, _nakamaSystem);

            _controller = new GachaController(_nakamaSystem, _inventorySystem, _economySystem, _statsSystem);
        }
        ...
        public void Constructor_NullNakamaSystem_ThrowsArgumentNullException()
        {
            ...
            {
                new GachaController(null, _inventorySystem, _economySystem, _statsSystem);
            });
        }
        ...
        public void Constructor_NullInventorySystem_ThrowsArgumentNullException()
        {
            Assert.Throws<ArgumentNullException>(() =>
            {
                new GachaController(_nakamaSystem, null, _economySystem, _statsSystem);
            });
        }
        ...
        public void Constructor_NullEconomySystem_ThrowsArgumentNullException()
        {
            Assert.Throws<ArgumentNullException>(() =>
            {
                new GachaController(_nakamaSystem, _inventorySystem, null, _statsSystem);
            });
        }
        ...
        public async Task RefreshInventoryAsync_ReturnsInventoryItems()
        {
             ...
             var result = _controller.InventoryItems;
        ...
        [Test]
        public async Task ConsumeItemAsync_GachaTicket_ReturnsNonNullWonItemId()
        {
            await _controller.LoadItemCodexAsync();
            await _controller.RefreshInventoryAsync();
            ...
            for (int i = 0; i < _controller.CodexItems.Count; i++)
            {
                if (_controller.CodexItems[i].Id == "gacha_ticket")
                {
                    ticketIndex = i;
                    break;
                }
            ...
            if (ticketIndex == -1)
            {
                Assert.Inconclusive("gacha_ticket not found in codex.");
                return;
            }

            await _controller.GrantItemAsync(ticketIndex, 1);
            await _controller.RefreshInventoryAsync();

            IInventoryItem ticket = null;
            foreach (var item in _controller.InventoryItems)
            {
            ...
            if (ticket == null)
            {
                Assert.Inconclusive("Granted gacha_ticket not found in inventory.");
                return;
            }

            _controller.SelectItem(ticket);

            var wonItemId = await _controller.ConsumeItemAsync(1, false);

            Assert.IsNotNull(wonItemId);
            Assert.IsNotEmpty(wonItemId);
            Debug.Log($"Won item ID: {wonItemId}");
        }

        [Test]
        public async Task ConsumeTenItemsAsync_GachaTicket_ReturnsTenItemIds()
        {
            await _controller.LoadItemCodexAsync();
            await _controller.RefreshInventoryAsync();
            ...
            await _controller.GrantItemAsync(ticketIndex, 10);
            await _controller.RefreshInventoryAsync();

            IInventoryItem ticket = null;
            foreach (var item in _controller.InventoryItems)
            {
                if (item.Id == "gacha_ticket") { ticket = item; break; }
            }
            ...
            if (ticket.Count < 10)
            {
                Assert.Inconclusive($"Expected at least 10 gacha_ticket in inventory, found {ticket.Count}.");
                return;
            }
            ...
            Assert.IsNotNull(wonItemIds);
            Assert.AreEqual(10, wonItemIds.Length);
            foreach (var id in wonItemIds)
            {
                Assert.IsNotNull(id);
                Assert.IsNotEmpty(id);
                Debug.Log($"Won item ID: {id}");
            }
        }
